 ...
 * @author chaostone
 */
class CertExamSignupConfig extends LongId with Coded with Named with InstantRange {
  /** 考试类型 */
  var category: CertificateCategory = _
  /** 教学项目 */
  var project: Project = _
  /** 报名科目设置 */
  var settings: mutable.Buffer[CertExamSignupSetting] = new mutable.ArrayBuffer[CertExamSignupSetting]
  /** 在规定的时间段内,是否可以开放 */
  var opened = false
  /** 通知 */
  var notice: String = _
  ...
  def getSetting(subject: CertificateSubject): Option[CertExamSignupSetting] = {
    settings.find(_.subject == subject)
  }

  def addSetting(setting: CertExamSignupSetting): Unit = {
    settings += setting
    setting.config = this
  }
