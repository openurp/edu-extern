 ...
 * @author chaostone
 */
class CertExamSignupSetting extends LongId {
  /** 报名科目 */
  var subject: CertificateSubject = _
  /** 报名设置(期号) */
  var config: CertExamSignupConfig = _
  /** 要求报名费 */
  ...
  /** 报名条件 */
  var scopes: mutable.Buffer[CertExamStdScope] = new mutable.ArrayBuffer[CertExamStdScope]
  ...
  def isTimeCollision(setting: CertExamSignupSetting): Boolean = {
    examOn match {
      case None => false
      case Some(e1) =>
