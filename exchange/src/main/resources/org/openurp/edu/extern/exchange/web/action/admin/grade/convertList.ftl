[#ftl]
[@b.head/]
  [@b.toolbar title = "<span style=\"color:blue\">" + exchangeGrade.exchangeStudent.std.user.name + "（<span style=\"padding-left: 1px; padding-right: 1px\">" + exchangeGrade.exchangeStudent.std.user.code + "</span>）于" + exchangeGrade.acquiredOn?string("yyyy-MM-dd") +"获得“"+ exchangeGrade.courseName + "”</span>课程及认定认定明细"]
  ...
  [@b.grid items=convertedGrades?sort_by(["course", "code"]) var="courseGrade" sortable="false"]
    [@b.gridbar]
      bg.form.addInput(action.getForm(), "exchangeGrade.id", "${exchangeGrade.id}");
      bar.addItem("取消", action.single("removeCourseGrade", "确认要取消认定吗？"), "action-edit-delete");
    [/@]
    [@b.row]
      [@b.boxcol type="radio"/]
      ...
      [@b.col title="成绩" property="score"]${(courseGrade.scoreText)!"--"}[/@]
 ...
 [@b.toolbar title = "添加新的认定课程"]
    bar.addItem("认定", function() {
      ...
      var form = document.exchangeGradeDistributeForm;
  ...
  [/@]
  [@b.form name="exchangeGradeDistributeForm" action="!convert" target="exchangeGrades"]
  <table class="gridtable">
    ...
    <tbody><input type="hidden" name="exchangeGrade.id" value="${exchangeGrade.id}"/>
