...
[@b.form name="signupListForm" action="!search" target="signupList"]
    [@b.grid items=signups var="signup"]
        ...
        [@b.row]
            ...
            [@b.col property="certificate.category.name" title="考试类型" width="9%"/]
            [@b.col property="certificate.name" title="报名科目" width="14%"/]
...
<script>
    ...
    function exportList(form){
        bg.form.addInput(form, "titles", "std.code:学号,std.name:姓名,std.state.grade.code:年级,std.gender.name:性别,std.state.major.name:专业,"+
                      "std.state.major.code:专业代码,std.state.department.name:所属院系,certificate.category.name:考试类型,"+
                      "certificate.name:报名科目,semester.schoolYear:学年,semester.name:学期,feeOfSignup:报名费,"+
    ...
    function exportCET(form){
        bg.form.addInput(form, "keys", "certificate.name,std.name,std.gender.name,std.code,std.person.idType.name,std.person.code,std.level.name,std.duration,std.beginOn,std.state.grade.code,std.state.department.name,std.state.major.name,std.squad.name,std.squad.code");
    ...
    function exportComputer(form){
        bg.form.addInput(form, "keys", "std.code,std.name,std.gender.name,std.person.idType.name,std.person.code,std.state.department.name,std.state.major.discipline.name,std.state.major.name,std.beginOn,std.duration,std.squad.name,certificate.name,payState.name");
    ...
    function exportPETS(form){
        bg.form.addInput(form, "keys", "std.project.category.name,std.person.idType.name,std.person.code,std.name,std.person.gender.name,std.person.birthday,std.project.school.name,std.level.name,std.enrollYear,std.graduateStatus,dummy1,certificate.name,std.examineeCode,dummy2,std.code,std.state.grade,std.state.department.name,updatedAt,ip");
