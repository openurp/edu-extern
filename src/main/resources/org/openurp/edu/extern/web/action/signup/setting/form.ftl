...
[@b.form name="settingForm" action=b.rest.save(setting) theme="list"]
    ...
    [@b.date label="考试日期"  name="setting.examOn" required="false" value=setting.examOn! style="width:150px"/]
    [@b.date label="开始时间" name="setting.examBeginAt" id="sTime" maxDate="#F{$dp.$D(\\'eTime\\')}" value="${(setting.examBeginAt)!}" format="HH:mm" required="false" /]
    [@b.date label="结束时间" name="setting.examEndAt" id="eTime" minDate="#F{$dp.$D(\\'sTime\\')}" value="${(setting.examEndAt)!}" format="HH:mm" required="false" /]
